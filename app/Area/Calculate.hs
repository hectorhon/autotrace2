...
calculate calcOpts connStr (Entity aid area) = do
  ...
  runNoLoggingT $ withPostgresqlConn connStr $ lift . runReaderT (do
    demand <- case areaParent area of
      ...
      Just parent -> do
        parentDemand <- intervalsDuring start end Demand parent
...
cleanRange :: UTCTime -> UTCTime -> Key Area -> MetricType -> SqlPersistT IO ()
cleanRange start end aid metricType = do
  ...
  forM_ exceedRanges (\ (Entity kr r) -> do
    ...
    insert_ $
      AreaInterval (areaIntervalArea r) end (areaIntervalEnd r) metricType)

writeIntervals :: UTCTime -> UTCTime -> Key Area -> MetricType -> [TSInterval]
               -> SqlPersistT IO ()
writeIntervals start end aid metricType intervals =
  forM_ (fmap g intervals) f where
    g (s, e) = ( max (addUTCTime s refTime) start
               , min (addUTCTime e refTime) end   )
    insert_' s e = insert_ $ AreaInterval aid s e metricType
    f (s, e)
      | s == start && e < end = do
        numAffected <- E.updateCount $ \ u -> do
          E.set u [ AreaIntervalEnd E.=. E.val e ]
          E.where_ (       (u E.^. AreaIntervalArea)     E.==. E.val aid
                     E.&&. (u E.^. AreaIntervalCategory) E.==. E.val metricType
